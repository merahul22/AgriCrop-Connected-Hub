...
function AuctionItem({ item, contract, accounts, web3, walletAddress }) {
  ...
  return (
    <div className={isEnded}>
      ...
      <h2>{item.name}</h2>
      {/* <p className="bid-info">Quantity:{item.quantity}</p> */}
      <p className="bid-info">Current Bid Price: {Number(price)*5000} Rs </p>
      <p className="buyout-info">Buyout Price: {web3.utils.fromWei(item.buyoutPrice, 'ether')*5000} Rs</p>
